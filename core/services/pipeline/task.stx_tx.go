...
type STXTxTask struct {
	...
	db        *gorm.DB
	config    Config
	keyStore  STXKeyStore
	txManager STXTxManager
}
...
type STXKeyStore interface {
	GetRoundRobinAddress(addrs ...common.Address) (common.Address, error)
}

type STXTxManager interface {
	CreateSTXTransaction(db *gorm.DB, fromAddress, toAddress common.Address, payload []byte, gasLimit uint64, meta interface{}, strategy bulletprooftxmanager.TxStrategy) (etx bulletprooftxmanager.EthTx, err error)
}

var _ Task = (*ETHTxTask)(nil)

func (t *STXTxTask) Type() TaskType {
	return TaskTypeSTXTx
}

func (t *STXTxTask) Run(_ context.Context, vars Vars, inputs []Result) (result Result) {
	...
	_, err = t.txManager.CreateSTXTransaction(t.db, fromAddr, common.Address(toAddr), []byte(data), uint64(gasLimit), &txMeta, strategy)
	if err != nil {
